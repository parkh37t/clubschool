...
def build_pptx(path):
    ...
    def note(s, script, talk="", time=""):
        # 발표자 노트: 스크립트(말할 요지) + 진행 팁 + 예상 소요. 회의 진행용.
        tf = s.notes_slide.notes_text_frame
        tf.text = script
        if talk:
            p = tf.add_paragraph(); p.text = "▶ 진행: " + talk
        if time:
            p = tf.add_paragraph(); p.text = "⏱ 소요: " + time
    ...
    note(s,
         "오늘 보고 목적: 컨버전스 실적관리 앱을 여러 회사가 쓰는 SaaS 제품으로 키우는 안을 승인받는 것. "
         "이미 우리 조직 56명에서 돌아가는 걸 봤고, 그 엔진을 그대로 상품화합니다.",
         "제품명은 가칭 Converge. 12장, 뒤 3장이 결정 요청.", "0.5분")
    ...
    note(s,
         "한 문장으로: 엑셀로 흩어진 가동률을 5분 만에 '금액'으로 보여주는 도구입니다. "
         "새로 만드는 게 아니라 검증된 엔진을 멀티테넌트로 여는 것이라 리스크가 낮습니다.",
         "'2개 분기 내 파일럿 5~10개'가 이번 승인의 성공 지표.", "1분")
    ...
    note(s,
         "고객이 겪는 손실 3가지. 핵심은 세 번째 — 가동률(%)이 재무 언어(돈)로 번역이 안 됩니다. "
         "그래서 경영 보고가 안 되고, 우리는 그 번역을 자동화합니다.",
         "Aha 한 줄을 천천히 읽어 청중이 자기 조직에 대입하게 만든다.", "1분")
    ...
    note(s,
         "누구에게 파느냐. 30~120명이 스윗스팟 — 엑셀은 무너지고 해외 PSA(Kantata 등)는 과합니다. "
         "이 '빈 시장'이 우리 진입 근거. 우리 조직이 정확히 이 프로필이라 첫 고객 이해가 깊습니다.",
         "Anti-ICP를 분명히 해서 '아무나 판다'는 인상을 피한다.", "1분")
    ...
    note(s,
         "제품은 3기둥. 강조점: ①에서 기존 코드 대부분을 재사용하므로 개발 리스크가 작고, "
         "③ AI는 재무 숫자를 계산하지 않고 서사만 담당해 '틀린 숫자' 리스크를 구조적으로 차단합니다.",
         "차별화 3축이 경쟁사 대비 우리의 '왜 우리냐' 답.", "1.5분")
    ...
    note(s,
         "MVP는 '한 회사가 도움 없이 가입→세팅→유휴비용 대시보드 도달'까지. M0는 빨간 글씨 그대로 "
         "협상 불가 선행조건 — 안 고치면 편집해도 숫자가 목데이터로 고정돼 재무 신뢰가 무너집니다.",
         "제외 목록으로 '이번엔 안 한다'를 분명히 해 범위 확장을 방어.", "1.5분")
    ...
    note(s,
         "기술 요지 한 장. 핵심 메시지 2개: (1) 테넌트 격리는 DB(RLS)+앱 2중 방어라 타사 데이터 노출을 막고, "
         "(2) useDataStore 시그니처를 유지해 화면 코드를 거의 안 바꾸고 저장소만 교체합니다.",
         "비개발 경영진에겐 '격리'와 '단가 보호'만 남겨도 충분.", "1.5분")
    ...
    note(s,
         "AI는 '있으면 좋다'가 아니라 리텐션 훅입니다. F5 자동 브리핑이 매주 이메일로 변화를 요약해 재방문을 만들고, "
         "F4 자연어 질의가 경쟁 차별점. 예측군은 데이터가 6~12개월 쌓여야 의미 있어 뒤로 뺐습니다.",
         "'재무 숫자는 앱, 서사는 Claude' 원칙을 여기서도 반복.", "1분")
    ...
    note(s,
         "과금은 좌석당(관리 대상 인원)×티어. 뷰어는 무료라 조직 안에서 자연 확산됩니다. "
         "Pro ₩9,900, Business ₩19,000, AI 애드온 +₩4,000 — 파일럿은 무료로 지불의사부터 검증합니다.",
         "가격은 '검증 후 조정' 전제임을 밝혀 확정 압박을 피한다.", "1분")
    ...
    note(s,
         "일정은 3구간. 올해 하반기 MVP(파일럿), 내년 상반기 유료화(30개), 하반기 스케일. "
         "의존성 순서가 고정 — M0부터 순서대로 가야 하고, 각 구간엔 격리 자동 테스트라는 통과 게이트가 있습니다.",
         "'언제 매출이 나오나' 질문엔 v1(2027 상반기 유료 30개)로 답.", "1분")
    ...
    for t in [
        ...
        ln(tf2, "• " + t, 15, FG, sp=6)
    note(s,
         "이번 회의의 핵심 요청 장. 리스크 중 A3(정합성=M0)와 A4(격리)가 치명군. "
         "그리고 아래 6건은 팀이 권고안을 냈지만 경영이 확정해야 다음 단계 확정이 가능합니다.",
         "6건 중 최소 ①요금 ②이관 범위 두 건은 오늘 방향이라도 받아낸다.", "2분")
    ...
    note(s,
         "마무리 & 요청: 오늘 승인만 나면 팀은 1번(M0 버그 수정)부터 바로 착수합니다. "
         "이건 새 개발이 아니라 이미 도는 엔진을 고쳐 얹는 일이라 위험이 낮다는 점을 다시 강조.",
         "승인·결정 6건·파일럿 예산 3가지를 클로징으로 요청.", "1분")
